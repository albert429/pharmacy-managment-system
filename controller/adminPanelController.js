app.controller("AdminPanelController", function ($scope, $q, PharmacyService) {
  ...
  $q.all([
    ...
    .then(function (results) {
      ...
      $scope.stats.admins = users.filter(function (u) {
        return u.role === "admin";
      }).length;
      $scope.stats.cashiers = users.filter(function (u) {
      ...
      $scope.stats.paid = invoices.filter(function (inv) {
        return inv.payment_status === "paid";
      }).length;
      $scope.stats.unpaid = invoices.filter(function (inv) {
      ...
      $scope.staff = users
        .map(function (user) {
          ...
          return {
            ...
            revenue: mine.reduce(function (s, inv) {
              return s + (parseFloat(inv.total_amount) || 0);
            }, 0),
          };
        })
        .sort(function (a, b) {
