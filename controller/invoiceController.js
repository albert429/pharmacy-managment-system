app.controller('InvoiceController', function ($scope, $q, $rootScope, PharmacyService, AuthService) {
  ...
  // ── Form state ─────────────────────────────────────────────────────────────
  $scope.newInvoice = { customer_id: null, discount: 0, payment_status: 'unpaid', items: [] };
  $scope.newItem    = { medicine_id: null, quantity: 1, unit_price: 0, maxQty: 0 };
  ...
  $q.all([
    ...
    $scope.customers.forEach(function (c) { $scope.customersMap[c.customer_id] = c; });
    $scope.medicines.forEach(function (m) { $scope.medicinesMap[m.medicine_id] = m; });
  });
  ...
  $scope.addItem = function () {
    var med = $scope.medicinesMap[$scope.newItem.medicine_id];
    if (!med || !$scope.newItem.quantity || $scope.newItem.quantity <= 0) return;
    if ($scope.newItem.quantity > med.quantity) {
      $scope.itemError = 'Quantity exceeds available stock (' + med.quantity + ')';
      return;
    }
  ...
  $scope.grandTotal = function () {
    return Math.max(0, $scope.subtotal() - (parseFloat($scope.newInvoice.discount) || 0));
  };

  // ── Submit invoice ──────────────────────────────────────────────────────────
  $scope.submitInvoice = function () {
    if (!$scope.newInvoice.customer_id || $scope.newInvoice.items.length === 0) return;
    ...
    PharmacyService.addInvoice(invoicePayload)
      ...
      .then(function () {
        ...
        var updates = savedItems.map(function (item) {
          ...
          return PharmacyService.editMedicine(item.medicine_id, { quantity: Math.max(0, newQty) });
        });
      ...
      .then(function (res) {
        $scope.medicinesMap = {};
        (res.data || []).forEach(function (m) { $scope.medicinesMap[m.medicine_id] = m; });
